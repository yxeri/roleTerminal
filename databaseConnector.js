...
function createRoom(sentRoom, sentUser, callback) {
  ...
  if (sentUser && 11 > sentUser.accessLevel) {
    query = {
      $or : [
...
function populateDbRooms(sentRooms, user) {
  ...
  Room.find().lean().exec(function(err, rooms) {
    ...
    } else if (null === rooms || 3 > rooms.length) {
