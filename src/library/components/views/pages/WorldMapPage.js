...
import { ChangeTypes } from '../../../react/SocketManager';
...
class WorldMapPage extends BaseView {
  ...
  realignMap({
    ...
    if (!useDefaultCoordinates && markers) {
      markers.forEach((marker) => {
        ...
        bounds.extend(new window.google.maps.LatLng(coordinates.latitude, coordinates.longitude));
      });
    } else {
      ...
      bounds.extend(new window.google.maps.LatLng(cornerCoordinates.upperLeft.latitude, cornerCoordinates.upperLeft.longitude));
      bounds.extend(new window.google.maps.LatLng(cornerCoordinates.bottomRight.latitude, cornerCoordinates.bottomRight.longitude));
    }
  ...
  startMap() {
    ...
    if (typeof window.google === 'undefined' || typeof window.MarkerClusterer === 'undefined' || typeof MapLabel === 'undefined') {
    ...
    this.worldMap = new window.google.maps.Map(this.element, {
    ...
    this.clusterer = new window.MarkerClusterer(this.worldMap, [], this.clusterStyle);
    ...
    window.google.maps.event.addListener(this.worldMap, 'drag', () => {
      MapObject.hideLeftClickBox();
      MapObject.hideRightClickBox();
    });

    window.google.maps.event.addListener(this.worldMap, 'zoom_changed', () => {
