...
class MapPolygon extends MapObject {
  constructor({
    ...
    const allPoints = [new window.google.maps.LatLng(latestCoordinates.latitude, latestCoordinates.longitude)]
      .concat((latestCoordinates.extraCoordinates || []).map((coords) => new window.google.maps.LatLng(coords.latitude, coords.longitude)));
    ...
    const options = {
      zIndex,
      paths: new window.google.maps.MVCArray(allPoints),
      ...style,
    };
    ...
    super({
      ...
      mapObject: new window.google.maps.Polygon(options),
    });
  }
  ...
  setCurrentCoordinates({ coordinates }) {
    const allPoints = [new window.google.maps.LatLng(coordinates.latitude, coordinates.longitude)]
      .concat(coordinates.extraCoordinates.map((coords) => new window.google.maps.LatLng(coords.latitude, coords.longitude)));

    super.setCurrentCoordinates({ coordinates });

    this.mapObject.setPath(new window.google.maps.MVCArray(allPoints));
  }
}
