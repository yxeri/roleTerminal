...
function addGMapsHoverListeners({
  ...
  window.google.maps.event.addListener(element, 'mouseover', (event) => {
    isHovering = true;
  ...
  if (outFunc) {
    window.google.maps.event.addListener(element, 'mouseout', (event) => {
      isHovering = false;
    ...
    if (shouldOutOnDrag) {
      window.google.maps.event.addListener(element, 'drag', outFunc);
    }
  }
}
...
class MapObject {
  ...
  getCenter() {
    ...
    bounds.extend(new window.google.maps.LatLng(this.currentCoordinates.latitude, this.currentCoordinates.longitude));
  ...
  showPositionRightClickBox({
    ...
    const { x, y } = this.overlay.getProjection().fromLatLngToContainerPixel(new window.google.maps.LatLng(coordinates.latitude, coordinates.longitude));
