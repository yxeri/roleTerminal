...
import socketManager, { ChangeTypes } from '../react/SocketManager';
...
class BaseData {
  constructor({
    ...
    if (emitTypes) {
      const func = ({ data }) => {
        ...
        paramsToEmit[this.objectTypes.one] = changeType === ChangeTypes.REMOVE
  ...
  updateObject({
    ...
    socketManager.emitEvent(event || this.updateEvents.one, params, ({ error, data }) => {
      ...
      const object = data[this.objectTypes.one];
      const updatedObject = this.objects[object.objectId]
        ? Object.assign(this.objects[object.objectId], object)
        : object;

      this.objects[object.objectId] = updatedObject;
      dataToReturn[this.objectTypes.one] = updatedObject;
